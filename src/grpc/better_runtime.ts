...
const walltime_clock_layer = make_annotation_layer(
    "walltime", 
    (get_name: () => string, 
    ...
    summarize_self: () => string[]) => {
        ...
        function get_procedure(name: string, arity: number): any | undefined {
            return  (base: any, ...timestamps: any[]) => {
...
const get_wallclock_timestamp = layer_accessor(walltime_clock_layer)
const has_walltime_clock_layer = walltime_clock_layer.has_value
    ...
    define_generic_procedure_handler(
        ...
        (set: BetterSet<LayeredObject<any>>) => {
            if (length(set) === 0) {
                ...
                const result = reduce(rest(set), (acc: LayeredObject<any>, value: LayeredObject<any>) => {
                      if (is_contradiction(acc)) {
                        ...
                        // console.log("walltime_less_than")
                        // console.log(value.summarize_self())
                        // console.log(acc.summarize_self())
                        return merge_vector_clock_from(acc, value)
                      }
                      else {
...
export const update_specialized_reactive_value = (cell: Cell<any>, identifier: string, value: any) => {
    ...
    if ((is_layered_object(strongest)) && (has_source_clock_channel(strongest, identifier))) {
        ...
        console.log("update_specialized_reactive_value: no source clock channel")
        update_cell(cell,
            construct_layered_datum(
                value,
                vector_clock_layer,
