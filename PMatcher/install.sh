...
echo -e "${BLUE}🚀 Setting up PMatcher workspace...${NC}\n"

# Get the directory where this script is located
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Determine workspace root and project location
if [ -d "$SCRIPT_DIR/PMatcher" ] && [ -f "$SCRIPT_DIR/PMatcher/package.json" ]; then
    WORKSPACE_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
    PROJECT_PATH="PMatcher/PMatcher"
    echo -e "${BLUE}📁 Detected cloned repository structure${NC}"
elif [ -f "$SCRIPT_DIR/package.json" ] && grep -q '"name": "pmatcher"' "$SCRIPT_DIR/package.json" 2>/dev/null; then
    WORKSPACE_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
    PROJECT_PATH="PMatcher"
...
if [ ! -f "package.json" ] || ! grep -q '"name".*workspace' package.json 2>/dev/null; then
    ...
    if [ ! -d "$PROJECT_PATH" ]; then
        echo -e "${RED}❌ Error: Cannot find PMatcher at $PROJECT_PATH${NC}"
        exit 1
    fi
...
elif [ -d "PMatcher/PMatcher/test" ]; then
    TEST_PATH="PMatcher/PMatcher/test"
elif [ -d "PMatcher/test" ]; then
    TEST_PATH="PMatcher/test"
else
...
if [ -n "$PROJECT_PATH" ]; then
    ...
    echo -e "  • Run tests: ${YELLOW}cd PMatcher && bun test${NC}"
fi
