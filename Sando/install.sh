...
echo -e "${BLUE}🚀 Setting up Sando workspace...${NC}\n"

# Get the directory where this script is located
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Determine workspace root and project location
if [ -d "$SCRIPT_DIR/Sando" ] && [ -f "$SCRIPT_DIR/Sando/package.json" ]; then
    WORKSPACE_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
    PROJECT_PATH="Sando/Sando"
    echo -e "${BLUE}📁 Detected cloned repository structure${NC}"
elif [ -f "$SCRIPT_DIR/package.json" ] && grep -q '"name": "sando-layer"' "$SCRIPT_DIR/package.json" 2>/dev/null; then
    WORKSPACE_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
    PROJECT_PATH="Sando"
...
if [ ! -f "package.json" ] || ! grep -q '"name".*workspace' package.json 2>/dev/null; then
    ...
    if [ ! -d "$PROJECT_PATH" ]; then
        echo -e "${RED}❌ Error: Cannot find Sando at $PROJECT_PATH${NC}"
        exit 1
    fi
...
# Determine the test path (Sando uses Test/ not test/)
if [ -d "$PROJECT_PATH/Test" ]; then
    TEST_PATH="$PROJECT_PATH/Test"
elif [ -d "Sando/Sando/Test" ]; then
    TEST_PATH="Sando/Sando/Test"
elif [ -d "Sando/Test" ]; then
    TEST_PATH="Sando/Test"
else
...
if [ -n "$PROJECT_PATH" ]; then
    ...
    echo -e "  • Run tests: ${YELLOW}cd Sando && bun test${NC}"
fi
