...
In this repo, `Propogator/DataTypes/Premises.ts` and `PremiseMetaData` implement that idea: premises are registered with **roots**, and toggling belief runs **`WAKE_CELLS_FOR_PREMISE`** (targeted `testContent` via `CellValueStore.premise_index` for **clock-channel** premises) plus **`ALERT_ALL_AMBS`**. `FORCE_UPDATE_ALL_CELLS` is no longer dispatched from `wake_up_roots` (escape hatch only). Support-layer-only premise tags are still not indexed; see `Propogator/docs/TODO-PREMISE-INDEX-SUPPORT-LAYER.md`. `is_premises` remains **quite slow** in code comments.
...
| Premise retraction | `Premises.ts`, `PremiseMetaData.ts`, `WAKE_CELLS_FOR_PREMISE` | Targeted wake for **clock-channel** indexed cells; support-layer gap in [TODO-PREMISE-INDEX-SUPPORT-LAYER.md](../../../Propogator/docs/TODO-PREMISE-INDEX-SUPPORT-LAYER.md) |
...
- [Premises, retraction, and global wake-up](./PREMISES-RETRACTION.md) — `ValueSet` / `Premises` / `PremisesSource` / `TemporaryValueSet`, and why cheap retraction matters for this roadmap.
