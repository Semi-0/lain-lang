...
蘭因乃以 propagator（傳播子）為本之高表達程式語言暨即時編碼執行環境，用於分散式計算。其以漸進式編譯為骨幹，視程式為可活化之資料：定義一經更易，系統即自動重編。其義理上，編譯器直將程式降格為「傳播子圖」（propagator graph）；且編譯流程本身亦以傳播子編織，使編譯亦成反應式、漸進式之運作。閉包可熱置換而不失其狀態：你可於甲機改易函式定義，而其變更能即時傳播至諸連線同儕，並保全既有執行態。計算以 cell／propagator 之反應式網路運作，輸入一變，輸出自更新；分散情境中並以向量鐘（vector clocks）保證因果秩序之正確。
